#!/usr/bin/env python3
"""
OpenAI o4-mini Inference Script for SUMO Traffic Simulation
仅进行推理实验，不触发训练，记录Token Usage

使用OpenAI-Hub国内镜像站 (api.openai-hub.com)
...
快速运行:
1. 设置环境变量: export OPENAI_API_KEY="your-api-key-here"
2. 使用默认配置运行: python run_openai_o4mini_inference.py
3. 自定义参数运行: python run_openai_o4mini_inference.py --av-ratio 0.05 --batch-size 16

常用参数:
--av-ratio 0.02         # 从第一个路由文件抽取的车辆比例 (默认2%)
--batch-size 8          # 批处理大小 (默认8)
--model o4-mini         # 模型名称 (默认o4-mini)
--start-time 28800      # 仿真开始时间 (默认28800s = 8:00 AM)
--end-time 43200        # 仿真结束时间 (默认43200s = 12:00 PM)

注意:
- 使用OpenAI-Hub国内镜像站，API地址自动设置为 https://api.openai-hub.com/v1
- 与官方OpenAI接口完全兼容，仅需更改Host即可
- 在国内环境下可获得更稳定的访问速度
"""
...
def check_openai_api_key() -> bool:
    ...
    if not api_key:
        ...
        print("请先设置API密钥: export OPENAI_API_KEY='your-api-key-here'")
        print("获取API密钥: https://platform.openai.com/api-keys\n")
        return False

    if not api_key.startswith("sk-"):
        print("\n[警告] OPENAI_API_KEY格式可能不正确（通常以'sk-'开头）")

    print(f"[成功] 检测到OPENAI_API_KEY: {api_key[:10]}...")
    return True


def run_openai_o4mini_inference(
    ...
    """
    运行OpenAI o4-mini推理实验

    Args:
        model_name: OpenAI模型名称 (默认: o4-mini)
        batch_size: 批处理大小
    ...
    if output_dir is None:
        ...
        output_dir = f"logs/openai_o4mini_inference_{location}_{timestamp}"
    os.makedirs(output_dir, exist_ok=True)

    print(f"\n{'='*60}")
    print(f"OpenAI o4-mini 推理实验配置")
    print(f"{'='*60}")
    print(f"模型: {model_name}")
    print(f"API模式: OpenAI-Hub镜像站 (api.openai-hub.com)")
    ...
    try:
        if multi_agent:
            ...
            algo = MultiAgent_Route_Planning(
                ...
                llm_path_or_name=model_name,  # 使用o4-mini
                step_size=step_size,
                max_steps=max_steps,
        ...
        if hasattr(algo, 'llm_agent') and algo.llm_agent:
            ...
            token_file_traffic = os.path.join(output_dir, "token_usage_traffic_o4mini.json")
            token_file_regional = os.path.join(output_dir, "token_usage_regional_o4mini.json")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="使用OpenAI o4-mini进行SUMO交通仿真推理实验（不训练）"
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="o4-mini",
        help="OpenAI模型名称 (默认: o4-mini, 可选: o4-mini-2025-04-16)"
    )

...
import traci
from env_utils import parse_rou_file, get_edge_info, get_multiple_edges_info, get_congestion_level, get_autonomous_vehicle_observation, update_route, get_env_change_text
...
class Simulation:
    ...
    def run(self, llm_agent):
        ...
        pbar = tqdm(total=self.max_steps, desc="Simulation Progress", unit="steps")
        while step < self.max_steps:
            # Update edge information (批量优化)
            edges_info = get_multiple_edges_info(self.edges)
            for e in self.edges:
                if e in edges_info:
                    _, vehicle_num, vehicle_speed, vehicle_length, _ = edges_info[e]
                else:
